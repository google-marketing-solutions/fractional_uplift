...
def _get_version():
  with open(
      os.path.join(CURRENT_DIR, "fractional_uplift", "__init__.py")
  ) as fp:
    for line in fp:
      if line.startswith("__version__") and "=" in line:
        ...
        if version:
          return version
    raise ValueError(
        "`__version__` not defined in `fractional_uplift/__init__.py`"
    )
...
setuptools.setup(
    name="fractional_uplift",
    version=VERSION,
    python_requires=">=3.10",
    description=(
        "Package for performing uplift modeling that is aware of the cost of"
        " treatment."
    ),
